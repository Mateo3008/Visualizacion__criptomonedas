---
title: "Análisis de Criptomonedas: Visualización de Datos"
...
cat("📡 Extrayendo datos de criptomonedas...\n")
datos <- data.frame()

for (i in 0:9) {  # 10 páginas para tener suficientes datos
  cat("  Página", i+1, "de 10\n")
...
cat("\n✅ Extracción completada. Total:", nrow(datos), "criptomonedas\n")
```
...
ggplot(datos, aes(x = precio)) +
  ...
  labs(
    title = "Distribución de Precios de Criptomonedas",
...
ggplot(top15, aes(x = mcap_milesM, y = nombre, fill = cambio)) +
  ...
  labs(
    title = "Top 15 Criptomonedas por Capitalización",
...
ggplot(datos, aes(x = precio, y = volumen, color = cambio)) +
  ...
  labs(
    title = "Relación entre Precio y Volumen",
    x = "Precio USD (log)",
